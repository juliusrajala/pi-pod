...
const enabled = Bun.env.PI_POD_SYSTEMD_INTEGRATION === "1" && Bun.which("systemd-run") !== null && Bun.which("script") !== null;

test.if(enabled)("re-execs the normal launcher into a delegated tmux-compatible scope", async () => {
...
test.if(enabled)("keeps an interactive terminal attached inside the delegated scope", async () => {
  const child = Bun.spawn([
    "script",
    "-qefc",
