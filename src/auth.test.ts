...
test("rejects a symlinked auth subtree even when its leaf would be user-owned", async () => {
  ...
  await expect(createAuthProfile({ agent: "pi", name: "default", provider: "openai-codex" })).rejects.toThrow("escaped the wrapper-owned root");
...
test("stages only the selected host Pi credential and never writes it back", async () => {
  ...
  try {
    ...
    await writeFile(stage.authFile, JSON.stringify({
      "openai-codex": { type: "oauth", access: "changed", refresh: "changed", expires: 1_900_000_000_001 },
    }));
...
test("stages and persists only the selected Pi Codex credential", async () => {
  ...
  const stored = JSON.parse(await readFile((await loadAuthProfile("pi", "codex")).authFile, "utf8"));
...
test("bounds writable native auth staging separately from workspace storage", async () => {
  const profile = await createAuthProfile({ agent: "pi", name: "bounded", provider: "openai-codex" });
  const stage = await stageAuthProfile(profile);
  try {
    await writeFile(join(stage.agentStateDirectory, "native-cache"), "x".repeat(maxAuthStageBytes + 1));
    await expect(assertWorkspaceWithinLimit(stage.agentStateDirectory, maxAuthStageBytes, "Authentication staging")).rejects.toThrow("Authentication staging storage limit");
  } finally {
    await stage.cleanup();
  }
});
...
test("recovers a valid interrupted credential stage", async () => {
  const profile = await createAuthProfile({ agent: "pi", name: "default", provider: "openai-codex" });
  const lock = await acquireAuthProfile(profile);
  try {
    const stage = await stageAuthProfile(profile);
    await writeFile(stage.authFile, JSON.stringify({
...
test("does not unlock a dead PID while its named container still exists", async () => {
  ...
  const lock = await acquireAuthProfile(profile, { containerName });
  try {
    await writeFile(join(profile.directory, ".active", "owner.json"), JSON.stringify({
      version: 1,
      pid: 2_147_483_647,
    ...
    await withExistingContainer(containerName, async () => {
      await expect(removeAuthProfileLock("pi", "default")).rejects.toThrow("still mounted by container");
    });
  } finally {
    await lock.release();
  }
