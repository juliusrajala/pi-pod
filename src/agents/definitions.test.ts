...
test("Pi definition preserves fixed headless discovery controls and prompt transport", () => {
  ...
  const command = piDefinition.command({
    mode: "headless",
    promptPath: "/run/pi-pod-prompt",
    agentArgs: ["--model", "fixture"],
  });
  ...
  expect(
    piDefinition.preferenceArguments(
...
test("OpenCode definition retains bundled authentication and native prompt transport", () => {
  expect(
    openCodeDefinition.command({ mode: "interactive", agentArgs: ["--model", "openai/fixture"] }),
  ).toEqual(["opencode", "--pure", "--model", "openai/fixture"]);
  const command = openCodeDefinition.command({
    mode: "headless",
    promptPath: "/run/pi-pod-prompt",
  ...
  expect(command.slice(0, 5)).toEqual([
    "sh",
    "-c",
    expect.stringContaining("opencode run --pure --auto"),
    "pi-pod-prompt",
    "/run/pi-pod-prompt",
  ]);
  ...
  expect(
    openCodeDefinition.preferenceArguments(
      { model: { provider: "openai", id: "fixture" }, variant: "high" },
      [],
    ),
