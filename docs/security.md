...
- Host and API-token auth stage only one native credential in a fresh private per-container directory. API-token profiles validate safe provider-key syntax and the selected agent's generic native token envelope; provider/model support is left to the agent, whose errors are returned normally. Stages are source-only: pi-pod never writes host Pi/OpenCode state back or reconciles agent changes to an API-token profile. Interactive Pi package selections copy only explicitly listed files into bounded private snapshots mounted read-only. The source package is not mounted. Without a selection, legacy resource mounts remain restricted to canonical paths under `~/.pi/agent/extensions` or `~/.pi/agent/packages` and are read-only.
